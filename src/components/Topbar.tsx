...
export default function Topbar() {
  ...
  return (
    <header className="min-h-14 border-b bg-card flex items-center px-4 md:px-6 gap-3 flex-shrink-0">
      {/* Search */}
      <form
        ...
        onSubmit={e => {
          ...
          router?.push(
            '/workspace/' +
              (sections?.some(([, r]) => r === resource) ? resource : sections?.[0]?.[1] || 'projects') +
              '?search=' +
      ...
      >
        ...
        <select
          ...
          className="text-xs bg-transparent border-0 max-w-28"
          value={resource}
        ...
        />
        <button className="text-xs text-primary">Search</button>
      </form>
      ...
      <div className="ml-auto flex items-center gap-3">
        ...
        <div className="relative">
          ...
          {show && (
            <div className="absolute right-0 top-full mt-2 card shadow-lg p-4 w-64 z-40">
              <p className="text-sm break-all">{auth?.user?.email}</p>
