...
export default function MaintenanceAssessmentReportWizard({
  ...
  const [customSite, setCustomSite] = useState(Boolean(record?.site_name_custom));
  const [customPreparer, setCustomPreparer] = useState(Boolean(record?.prepared_by_name && !record?.prepared_by_employee_id));
  const [customStatus, setCustomStatus] = useState(Boolean(record?.status && !['DRAFT', 'IN_PROGRESS', 'COMPLETED'].includes(record.status)));
  const [customStatusValue, setCustomStatusValue] = useState(record?.status && !['DRAFT', 'IN_PROGRESS', 'COMPLETED'].includes(record.status) ? record.status : '');
  ...
  const projectOptions = [...projects.map((project) => ({ value: String(project.id), label: project.name || project.project_name || project.project_number })), { value: '__CUSTOM__', label: 'Enter a custom project name…' }];
  const siteOptions = [...projectSites.map((site) => ({ value: String(site.id), label: site.name || site.site_name || site.code || 'Site' })), { value: '__CUSTOM__', label: 'Enter a custom site name…' }];
  ...
  const statusOptions = [
    ...
    { value: 'COMPLETED', label: 'Completed' }, { value: '__CUSTOM__', label: 'Enter a custom status…' },
  ];
  ...
  const controls = <div className="space-y-4">
    ...
    <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-3">
      ...
      <label className="space-y-1"><span className="block font-medium">Project</span>{customProject ? <div className="space-y-1"><input autoFocus className="input-field" value={data.project_name_custom || ''} onChange={(e) => patch('project_name_custom', e.target.value)} placeholder="Enter a custom project name" maxLength={200} /><button type="button" className="text-primary underline" onClick={() => { setCustomProject(false); patch('project_name_custom', null); }}>Choose a registered project</button></div> : <SearchableSelect options={projectOptions} value={data.project_id} onChange={(value) => { if (value === '__CUSTOM__') { setCustomProject(true); patch('project_name_custom', ''); return; } setCustomProject(false); patch('project_name_custom', null); patch('project_id', value); patch('site_location_id', ''); setCustomSite(false); patch('site_name_custom', null); }} placeholder="Search projects" />}</label>
      <label className="space-y-1"><span className="block font-medium">Site / work location</span>{customSite ? <div className="space-y-1"><input autoFocus className="input-field" value={data.site_name_custom || ''} onChange={(e) => patch('site_name_custom', e.target.value)} placeholder="Enter a custom site or work location" maxLength={200} /><button type="button" className="text-primary underline" onClick={() => { setCustomSite(false); patch('site_name_custom', null); }}>Choose a registered site</button></div> : <SearchableSelect options={siteOptions} value={data.site_location_id} onChange={(value) => { if (value === '__CUSTOM__') { setCustomSite(true); patch('site_location_id', null); patch('site_name_custom', ''); return; } setCustomSite(false); patch('site_name_custom', null); patch('site_location_id', value); }} placeholder="Search sites" />}</label>
      ...
      <label className="space-y-1"><span className="block font-medium">Status</span>{customStatus ? <div className="space-y-1"><input autoFocus className="input-field" value={customStatusValue} onChange={(e) => setCustomStatusValue(e.target.value)} placeholder="Enter a custom status" maxLength={30} /><button type="button" className="text-primary underline" onClick={() => { setCustomStatus(false); setCustomStatusValue(''); patch('status', 'DRAFT'); }}>Choose a standard status</button></div> : <SearchableSelect options={statusOptions} value={data.status} searchable={false} onChange={(value) => { if (value === '__CUSTOM__') { setCustomStatus(true); setCustomStatusValue(''); return; } patch('status', value); }} placeholder="Select status" />}</label>
    </div>
  </div>;
  ...
  const tableSection = (sectionKey: string) => {
    ...
    return <div className="space-y-3" key={sectionKey}>
      ...
      {(data[sectionKey] || []).map((row: ReportRow, index: number) => <div key={`${sectionKey}-${index}`} className="grid gap-3 rounded-lg border border-border bg-muted/10 p-3 sm:grid-cols-2 lg:grid-cols-3">
        {definition.columns.map((column) => column.asset ? <div className="space-y-1" key={column.key}><span className="block font-medium">{column.label}</span>{row[column.key] === '__CUSTOM__' || (!row[column.key] && Boolean(row[sectionKey === 'equipment_fleet' ? 'equipment' : 'area_equipment'])) ? <div className="space-y-1"><input autoFocus className="input-field" value={row[sectionKey === 'equipment_fleet' ? 'equipment' : 'area_equipment'] || ''} onChange={(e) => patchRow(sectionKey, index, sectionKey === 'equipment_fleet' ? 'equipment' : 'area_equipment', e.target.value)} placeholder={`Enter custom ${sectionKey === 'equipment_fleet' ? 'equipment' : 'area / equipment'}`} /><button type="button" className="text-primary underline" onClick={() => { patchRow(sectionKey, index, column.key, ''); patchRow(sectionKey, index, sectionKey === 'equipment_fleet' ? 'equipment' : 'area_equipment', ''); }}>Choose registered equipment</button></div> : <SearchableSelect options={[{ value: '__CUSTOM__', label: 'Enter custom equipment…' }, ...assetOptions]} value={row[column.key] || ''} onChange={(value) => { if (value === '__CUSTOM__') { patchRow(sectionKey, index, column.key, '__CUSTOM__'); patchRow(sectionKey, index, sectionKey === 'equipment_fleet' ? 'equipment' : 'area_equipment', ''); return; } const asset = assets.find((item) => String(item.id) === String(value)); patchRow(sectionKey, index, column.key, value); if (asset) patchRow(sectionKey, index, sectionKey === 'equipment_fleet' ? 'equipment' : 'area_equipment', asset.name || asset.asset_name || asset.asset_number || ''); }} placeholder="Search equipment or enter custom" />}</div> : ((sectionKey === 'equipment_fleet' && column.key === 'equipment' || sectionKey === 'maintenance_assessment' && column.key === 'area_equipment') && (row.asset_id === '__CUSTOM__' || (!row.asset_id && Boolean(row[column.key])))) ? null : <label className={column.wide ? 'space-y-1 sm:col-span-2' : 'space-y-1'} key={column.key}><span className="block font-medium">{column.label}</span>{['observation_failure', 'action_taken_response', 'maintenance_focus', 'current_approach', 'purpose', 'justification'].includes(column.key) ? <textarea className="input-field min-h-20 resize-y" value={row[column.key] || ''} onChange={(e) => patchRow(sectionKey, index, column.key, e.target.value)} /> : <input className="input-field" value={row[column.key] || ''} onChange={(e) => patchRow(sectionKey, index, column.key, e.target.value)} />}</label>)}
  ...
  async function save() {
    ...
    try {
      const payload = { ...data, status: customStatus ? customStatusValue.trim() : data.status, project_id: data.project_id || null, site_location_id: data.site_location_id || null, prepared_by_employee_id: data.prepared_by_employee_id || null, equipment_asset_ids: equipmentIds, equipment_fleet: (data.equipment_fleet || []).map((row: ReportRow) => ({ ...row, asset_id: row.asset_id === '__CUSTOM__' ? null : row.asset_id })), maintenance_assessment: (data.maintenance_assessment || []).map((row: ReportRow) => ({ ...row, asset_id: row.asset_id === '__CUSTOM__' ? null : row.asset_id })) };
