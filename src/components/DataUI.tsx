...
export function Modal({name,title: modalTitle,onClose,children,className,error,footer}:{name?:string;title?:string;onClose:()=>void;children:ReactNode;className?:string;error?:string;footer?:ReactNode}){const headerTitle = name || modalTitle || 'Dialog'; const [mounted,setMounted]=useState(false);useEffect(()=>{setMounted(true);const listener=(e:KeyboardEvent)=>{if(e.key==='Escape')onClose();};window.addEventListener('keydown',listener);return()=>window.removeEventListener('keydown',listener);},[onClose]);if(!mounted)return null;return createPortal(<div className="fixed inset-0 z-[9999] bg-black/50 backdrop-blur-xs flex items-center justify-center p-0 sm:p-4 overflow-hidden" onClick={e=>{if(e.target===e.currentTarget)onClose();}}><section role="dialog" aria-modal="true" aria-label={headerTitle} className={`card shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] shadow-black/50 ring-1 ring-black/10 dark:ring-white/10 w-full h-full sm:h-auto max-w-full sm:max-w-4xl max-h-full sm:max-h-[90vh] rounded-none sm:rounded-2xl border-0 sm:border flex flex-col z-[10000] bg-white dark:bg-slate-900 overflow-hidden ${className||''}`}><header className="flex justify-between items-center p-4 sm:p-5 border-b shrink-0 bg-white dark:bg-slate-900 sticky top-0 z-10"><h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">{headerTitle}</h2><button type="button" autoFocus aria-label="Close dialog" className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-500 hover:text-slate-900 dark:hover:text-white" onClick={(e)=>{e.preventDefault();e.stopPropagation();onClose();}}><X size={20}/></button></header><div className="p-4 sm:p-6 overflow-y-auto flex-1 scrollbar-thin">{error && <div role="alert" className="sticky top-0 z-20 mb-4 border border-red-300 bg-red-50 p-3 text-sm text-red-900 shadow-sm"><strong className="block">Could not save changes</strong><p className="whitespace-pre-wrap break-words">{error}</p></div>}{children}</div>{footer&&<footer className="z-20 shrink-0 border-t border-slate-200 bg-white px-4 py-3 dark:border-slate-700 dark:bg-slate-900 sm:px-6">{footer}</footer>}</section></div>,document.body);}
