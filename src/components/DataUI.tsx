...
export function Modal({name,title: modalTitle,onClose,children,className,error,footer}:{name?:string;title?:string;onClose:()=>void;children:ReactNode;className?:string;error?:string;footer?:ReactNode}){
  const headerTitle = name || modalTitle || 'Dialog';
  ...
  useEffect(()=>{
    setMounted(true);
    const listener=(e:KeyboardEvent)=>{if(e.key==='Escape') handleClose();};
  ...
  return createPortal(
    <div
      className={`fixed inset-0 z-[99999] bg-black/70 backdrop-blur-xs flex items-center justify-center p-0 sm:p-4 overflow-hidden ${closing ? 'animate-modal-backdrop-out' : 'animate-modal-backdrop-in'}`}
    ...
    >
      <section
        ...
        className={`card shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] shadow-black/50 ring-1 ring-black/10 dark:ring-white/10 w-full h-[100dvh] sm:h-auto max-w-full sm:max-w-4xl sm:max-h-[90vh] rounded-none sm:rounded-2xl border-0 sm:border flex flex-col z-[100000] bg-white dark:bg-slate-900 overflow-hidden ${closing ? 'animate-modal-content-out' : 'animate-modal-content-in'} ${className||''}`}
      >
        <header className="flex justify-between items-center px-4 py-4 pt-[calc(1rem+env(safe-area-inset-top,0px))] sm:px-5 sm:py-5 border-b shrink-0 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md sticky top-0 z-10">
          <h2 className="text-base sm:text-lg font-bold text-slate-900 dark:text-white">{headerTitle}</h2>
          <button
            type="button"
            autoFocus
            aria-label="Close dialog"
            className="p-1.5 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 transition-colors text-slate-500 hover:text-slate-900 dark:hover:text-white"
