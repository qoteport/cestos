...
export function useData<T=any>(path:string|null){const [data,setData]=useState<T|null>(null);const [error,setError]=useState('');const [loading,setLoading]=useState(!!path);const [version,setVersion]=useState(0);useEffect(()=>{let active=true;setData(null);setError('');if(!path){setLoading(false);return;}setLoading(true);apiFetch<T>(path).then(d=>{if(active)setData(d);}).catch(e=>{if(active)setError(e.message);}).finally(()=>{if(active)setLoading(false);});return()=>{active=false;};},[path,version]);return{data,error,loading,reload:()=>setVersion(v=>v+1)};}
...
export function Modal({name,title: modalTitle,onClose,children,className,error}:{name?:string;title?:string;onClose:()=>void;children:ReactNode;className?:string;error?:string}){const headerTitle = name || modalTitle || 'Dialog'; const [mounted,setMounted]=useState(false);useEffect(()=>{setMounted(true);const listener=(e:KeyboardEvent)=>{if(e.key==='Escape')onClose();};window.addEventListener('keydown',listener);return()=>window.removeEventListener('keydown',listener);},[onClose]);if(!mounted)return null;return createPortal(<div className="fixed inset-0 z-[9999] bg-black/40 backdrop-blur-xs flex items-center justify-center p-3 sm:p-4" onClick={e=>{if(e.target===e.currentTarget)onClose();}}><section role="dialog" aria-modal="true" aria-label={headerTitle} className={`card shadow-[0_25px_60px_-15px_rgba(0,0,0,0.5)] shadow-black/50 ring-1 ring-black/10 dark:ring-white/10 w-full max-w-4xl max-h-[90vh] flex flex-col z-[10000] ${className||''}`}><header className="flex justify-between items-center p-5 border-b shrink-0"><h2 className="text-lg font-bold">{headerTitle}</h2><button type="button" autoFocus aria-label="Close dialog" className="p-1 rounded hover:bg-muted transition-colors text-muted-foreground hover:text-foreground" onClick={(e)=>{e.preventDefault();e.stopPropagation();onClose();}}><X size={20}/></button></header><div className="p-5 overflow-y-auto flex-1 scrollbar-thin">{error && <div role="alert" className="sticky top-0 z-20 mb-4 border border-red-300 bg-red-50 p-3 text-sm text-red-900 shadow-sm"><strong className="block">Could not save changes</strong><p className="whitespace-pre-wrap break-words">{error}</p></div>}{children}</div></section></div>,document.body);}
