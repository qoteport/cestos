...
function Reference({
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      apiFetch('/api/v1/' + route + '?page_size=100&search=' + encodeURIComponent(search))
        .then((d) => {
          if (active) {
            setOptions(rows(d));
            setError('');
  ...
  return (
    <div className="space-y-1">
      <input
        aria-label={'Find ' + title(field)}
        className="input-field text-xs"
        placeholder={'Find ' + title(field).toLowerCase() + '…'}
        value={search}
      ...
      <select
        ...
        onChange={(e) =>
          onChange(
            e.target.value,
            options.find((r) => r.id === e.target.value)
          )
        }
      >
        <option value="">Select {title(field).toLowerCase()}</option>
        {value && !options.some((r) => r.id === value) && <option value={value}>{value}</option>}
        {options.map((r) => (
...
function Fields({
  ...
  return (
    <>
      ...
      >
        {advanced ? 'Show fewer fields' : 'Additional details'}
      </button>
    </>
  );
}
