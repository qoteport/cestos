...
const groups = [
  ...
  },
  {
    ...
    links: [
      ['Assets', 'assets'],
      ['Available equipment', 'assets/available'],
      ['Open defects', 'asset-defects/open'],
      ['Critical defects', 'asset-defects/critical'],
      ['Expiring insurance', 'asset-insurance/expiring'],
      ['Expiring registrations', 'asset-registrations/expiring'],
  ...
  },
  {
    ...
    links: [
      'items', 'stores', 'requests', 'receipts', 'issues', 'transfers',
      'returns', 'reservations', 'stock-counts', 'adjustments', 'transactions',
      'stock', 'forecast', 'reorder-recommendations', 'categories', 'units',
      'suppliers', 'bins', 'stock-policies', 'lots', 'serials', 'custody',
    ].map(x => [
...
export default function Sidebar({
  ...
  const [open, setOpen] = useState<string[]>(
    groups
      .filter(g => path === g.href || g.links.some(([, r]) => path === '/workspace/' + r))
      .map(g => g.name)
  );
  ...
  return (
    <aside
      className={
        'border-r bg-card flex flex-col flex-shrink-0 sidebar-transition ' + (collapsed ?'w-16' : 'w-60')
      }
    >
      {/* Logo */}
      ...
      >
              ...
              <div key={g.name}>
                <div className="flex">
                  <Link
                    title={g.name}
                    href={g.href}
                    className={'flex flex-1 gap-3 items-center p-2.5 rounded text-sm ' + active(g.href)}
                  >
                    <g.icon size={18} />
                    {!collapsed && g.name}
                  </Link>
                  ...
                  <div className="ml-4 pl-3 border-l my-1 space-y-0.5">
                    {g.links
                      ...
                      .map(([label, resource]) => (
                        <Link
                          className={
                            'block px-2 py-1.5 rounded text-xs ' + active('/workspace/' + resource)
                          }
        ...
        {[
          ...
          <Link
            ...
            href={'/workspace/' + r}
          >
            {r === 'hr/notifications' ? <Bell size={18} /> : <User size={18} />}
            {!collapsed && label}
          </Link>
        ))}
      </nav>
