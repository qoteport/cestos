    ...
    <Modal title="Edit Shift Production Report" error={formErrors.shift} onClose={() => { if (!saving) onClose(); }}>
      <form onSubmit={submit} className="space-y-4 text-xs">
        ...
        <div className="flex justify-end gap-2 pt-3 border-t">
          <button type="button" disabled={saving} onClick={onClose} className="px-4 py-2 border rounded-lg hover:bg-muted font-medium">Cancel</button>
          <button disabled={saving || locked} className="px-4 py-2 bg-primary text-primary-foreground font-bold rounded-lg hover:bg-primary/90 transition">{saving ? 'Saving...' : 'Save Shift Report'}</button>
        </div>
      </form>
    </Modal>
  );
