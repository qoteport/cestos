    ...
    {queueOpen && <div className="fixed inset-0 z-[2147483647] flex items-end justify-center bg-slate-950/50 p-3 sm:items-center" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setQueueOpen(false); }}>
      <section className="flex max-h-[85vh] w-full max-w-2xl flex-col border border-slate-300 dark:border-slate-800 bg-white dark:bg-slate-900 shadow-2xl rounded-2xl overflow-hidden" role="dialog" aria-modal="true" aria-labelledby="offline-queue-title">
        ...
        <div className="min-h-0 space-y-3 overflow-y-auto p-4">
          {offlineWrites.length === 0 ? <p className="text-sm text-slate-600 dark:text-slate-400">All changes have synced.</p> : offlineWrites.map((item) => <article key={item.id} className="border border-slate-200 dark:border-slate-800 rounded-xl p-3 text-sm">
            <div className="flex flex-wrap items-start justify-between gap-2"><div className="min-w-0"><p className="font-semibold text-slate-900 dark:text-white">{item.method} · {item.path}</p><p className="mt-1 text-xs text-slate-500">{new Date(item.createdAt).toLocaleString()} · {item.state === 'failed' ? 'Needs attention' : 'Waiting to sync'}</p>{item.error && <p className="mt-2 text-red-700 dark:text-red-400">{item.error}</p>}</div><div className="flex gap-2">{item.state === 'failed' && <button onClick={() => { void retryOfflineWrite(item.id).then(() => getOfflineWriteQueue().then(setOfflineWrites)); }} className="border border-slate-300 dark:border-slate-700 rounded-lg px-2.5 py-1 font-semibold">Retry</button>}<button onClick={() => { void discardOfflineWrite(item.id).then(() => getOfflineWriteQueue().then(setOfflineWrites)); }} className="border border-red-200 dark:border-red-900/60 rounded-lg px-2.5 py-1 text-red-700 dark:text-red-400">Remove</button></div></div>
          </article>)}
        </div>
        <footer className="border-t border-slate-200 dark:border-slate-800 p-3 text-xs text-slate-500">Keep this browser profile and its site data until all queued changes have synced.</footer>
      </section>
    </div>}
  </>;
}
