...
export default function PreventiveMaintenanceWizard({
  ...
  return <Modal title={`${record ? 'Edit' : 'Preventive'} Maintenance Job Card`} onClose={onClose} className="sm:!h-[94vh] sm:!max-h-[94vh] sm:!w-[92vw] sm:!max-w-[1440px]">
    <div className="space-y-4 text-xs">
      ...
      <div className="flex justify-between border-t pt-3"><button type="button" className="btn-secondary" disabled={step === 0} onClick={() => setStep(step - 1)}>Back</button>{step < 3 ? <button type="button" className="btn-primary" disabled={step === 0 && !assetId && !(isCustomEquipment && customEquipment.trim())} onClick={() => setStep(step + 1)}>Next</button> : <button type="button" className="btn-primary" disabled={saving || (!assetId && !(isCustomEquipment && customEquipment.trim()))} onClick={save}>{saving ? 'Saving…' : 'Save Job Card'}</button>}</div>
      </>}
      {view === 'FREE_FLOW' && <div className="flex justify-end border-t pt-3"><button type="button" className="btn-primary" disabled={saving || (!assetId && !(isCustomEquipment && customEquipment.trim()))} onClick={save}>{saving ? 'Saving…' : 'Save Job Card & PDF'}</button></div>}
    </div>
