    ...
    <div className="space-y-4 text-xs">
      ...
      <div className="sticky bottom-0 -mx-4 -mb-4 sm:-mx-6 sm:-mb-6 p-3.5 sm:px-6 sm:py-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 z-10 flex flex-col-reverse sm:flex-row items-stretch sm:items-center justify-between gap-2.5 sm:gap-3 mt-4">
        ...
        <div className="flex flex-col-reverse sm:flex-row gap-2 w-full sm:w-auto sm:ml-auto">
          {step < 3 ? <button type="button" className="btn-primary w-full sm:w-auto text-xs" disabled={step === 0 && !assetId && !(isCustomEquipment && customEquipment.trim())} onClick={() => setStep(step + 1)}>Next</button> : <button type="button" className="btn-primary w-full sm:w-auto text-xs" disabled={saving || (!assetId && !(isCustomEquipment && customEquipment.trim()))} onClick={save}>{saving ? 'Saving…' : 'Save Job Card'}</button>}
        </div>
      </div>
      </>}
      {view === 'FREE_FLOW' && (
        <div className="sticky bottom-0 -mx-4 -mb-4 sm:-mx-6 sm:-mb-6 p-3.5 sm:px-6 sm:py-4 bg-white/95 dark:bg-slate-900/95 backdrop-blur-md border-t border-slate-200 dark:border-slate-800 z-10 flex justify-end gap-2 mt-4">
          <button type="button" className="btn-primary w-full sm:w-auto text-xs" disabled={saving || (!assetId && !(isCustomEquipment && customEquipment.trim()))} onClick={save}>{saving ? 'Saving…' : 'Save Job Card & PDF'}</button>
        </div>
      )}
    </div>
