    ...
    if (type === 'date') {
      return (
        <label className="block space-y-1 font-medium" key={key}>
          ...
          <AppDateTimePicker value={control[key] || ''} onChange={(val) => setControl({ ...control, [key]: val })} />
        </label>
      );
    }
    if (type === 'time') {
      return (
        <label className="block space-y-1 font-medium" key={key}>
          <span className="block">{label}</span>
          <AppDateTimePicker mode="time" value={control[key] || ''} onChange={(val) => setControl({ ...control, [key]: val })} />
        </label>
      );
    }
    return (
    ...
    <div className="space-y-4 text-xs">
      ...
      {view === 'FREE_FLOW' && <div className="max-h-[72vh] overflow-y-auto bg-slate-100 p-2 sm:p-4"><div className="freeflow-job-card mx-auto max-w-[1400px] space-y-3 bg-white p-3 shadow sm:p-6 text-slate-900">
        ...
        <section className="space-y-3"><h3 className="border-b bg-slate-100 p-2 font-black">A. PM CONTROL</h3><div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
          ...
          <label className="space-y-1 font-semibold">Date<AppDateTimePicker value={control.date || ''} onChange={(val) => setControl({ ...control, date: val })} /></label>
          ...
          <label className="space-y-1 font-semibold">Start Time<AppDateTimePicker mode="time" value={control.start_time || ''} onChange={(val) => setControl({ ...control, start_time: val })} /></label>
          <label className="space-y-1 font-semibold">Finish Time<AppDateTimePicker mode="time" value={control.finish_time || ''} onChange={(val) => setControl({ ...control, finish_time: val })} /></label>
        </div></section>
      ...
      {step === 2 && <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <label className="block space-y-1 font-medium"><span className="block">Total Labour Hrs</span><input value={service.total_labour_hours} onChange={(event) => setService({ ...service, total_labour_hours: event.target.value })} className="w-full border rounded-lg p-2 bg-background" /></label>
        <label className="block space-y-1 font-medium"><span className="block">Machine Down Hrs</span><input value={service.machine_down_hours} onChange={(event) => setService({ ...service, machine_down_hours: event.target.value })} className="w-full border rounded-lg p-2 bg-background" /></label>
        <label className="block space-y-1 font-medium sm:col-span-2"><span className="block">PM Result</span><SearchableSelect value={service.pm_result} onChange={(value) => setService({ ...service, pm_result: value })} options={['Passed', 'Passed with Observations', 'Further Maintenance Required', 'Machine Not Released'].map((value) => ({ value, label: value }))} placeholder="Select result..." /></label>
