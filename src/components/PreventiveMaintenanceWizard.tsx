...
export default function PreventiveMaintenanceWizard({
  ...
  >
    <div className="space-y-4 text-xs">
      ...
      {view === 'FREE_FLOW' && <div className="max-h-[72vh] overflow-y-auto bg-slate-100 p-2 sm:p-4"><div className="freeflow-job-card mx-auto max-w-[1400px] space-y-3 bg-white p-3 shadow sm:p-6 text-slate-900 [&_input]:rounded-none [&_textarea]:rounded-none [&_select]:rounded-none [&_div.rounded]:rounded-none [&_button]:rounded-none">
    ...
    {showCreateWorkOrder && (
      ...
      >
        <div className="space-y-4 text-xs">
          {workOrderError && <p role="alert" className="rounded-lg border border-destructive/40 p-3 text-destructive">{workOrderError}</p>}
          <section className="p-3 border rounded-xl space-y-3">
            <h4 className="font-bold uppercase tracking-wide">Target Equipment & Work Classification</h4>
            <div className="grid sm:grid-cols-2 gap-3">
              <label className="space-y-1">Target Equipment Asset *<SearchableSelect value={assetId} onChange={() => {}} options={assetOptions.filter((option) => option.value !== '__CUSTOM__')} placeholder={selected?.name || selected?.asset_number || 'Selected equipment'} disabled /></label>
          ...
          <section className="p-3 border rounded-xl space-y-3">
            <h4 className="font-bold uppercase tracking-wide">Scheduling & Specialist Assignment</h4>
            <div className="grid sm:grid-cols-2 gap-3">
              <label className="space-y-1">Scheduled Date *<AppDateTimePicker mode="date" required value={woForm.scheduled_date} onChange={(val) => setWoForm({ ...woForm, scheduled_date: val })} /></label>
