...
export default function OperationalExpenseSubmissionModal({ onClose, onSubmitted, projectId, initialPurchaseOrder }: { onClose: () => void; onSubmitted: (expense: Row) => void; projectId?: string; initialPurchaseOrder?: Row }) {
  ...
  return <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-slate-950/70 p-3 sm:p-4" onClick={(event) => { if (event.target === event.currentTarget) onClose(); }}>
    <section role="dialog" aria-modal="true" aria-label="Log operational expense" className="flex max-h-[92vh] w-full max-w-3xl flex-col overflow-hidden rounded-2xl border bg-white shadow-2xl dark:bg-slate-900">
      <header className="flex items-center justify-between border-b p-5"><div><h2 className="text-lg font-bold text-slate-900 dark:text-white">Submit Operational Expense Claim</h2><p className="mt-1 text-xs text-slate-500">Submit purchased items and invoice details to Finance.</p></div><button type="button" aria-label="Close" onClick={onClose} className="rounded-lg p-1 text-slate-500 hover:bg-slate-100 dark:hover:bg-slate-800">×</button></header>
      <form onSubmit={submit} className="space-y-4 overflow-y-auto p-5 text-xs">
        ...
        <section className="grid gap-3 rounded-xl border bg-slate-50 p-3 dark:bg-slate-800/40 sm:grid-cols-2">
          <label className="block space-y-1 font-semibold sm:col-span-2"><span className="block">Approved purchase order (optional)</span><SearchableSelect value={purchaseOrderId} onChange={(value) => {
            const po = purchaseOrders.find((row) => String(row.id) === value);
            if (po) applyPurchaseOrder(po);
            else setPurchaseOrderId(value);
          }} options={purchaseOrders.map((po) => ({ value: String(po.id), label: `${po.po_number} · ${po.currency} ${Number(po.total_amount || 0).toLocaleString()}` }))} placeholder="Link an approved purchase order…" /></label>
          <label className="block space-y-1 font-semibold sm:col-span-2"><span className="block">Pay to name *</span>{payeeId === '__NEW__' ? <><input autoFocus required className={inputClass} value={payName} onChange={(event) => setPayName(event.target.value)} placeholder="Enter the payee name" /><button type="button" className="mt-1 text-orange-700 underline" onClick={() => { setPayeeId(''); setPayName(''); }}>Choose a saved payee</button></> : <SearchableSelect value={payeeId} onChange={(value) => { if (value === '__NEW__') { setPayeeId('__NEW__'); setPayName(''); setPhone(''); setBank(''); return; } setPayeeId(value); const payee = payees.find((row) => String(row.id) === value); if (payee) { setPayName(payee.name || ''); setPhone(payee.phone || ''); setBank(payee.bank_account_details || ''); } }} options={payeeOptions} placeholder="Choose a saved payee or add a new one..." />}</label>
        ...
        <section className="space-y-3 rounded-xl border p-3"><div className="flex items-center justify-between"><h3 className="font-bold text-slate-800 dark:text-slate-100">Items purchased</h3><button type="button" onClick={() => setItems((rows) => [...rows, blankItem()])} className="font-bold text-orange-700">+ Add item</button></div>
          ...
          {items.map((item, index) => <div key={index} className="grid gap-2 rounded-lg border bg-slate-50 p-3 dark:bg-slate-800/40 sm:grid-cols-3"><label className="block space-y-1 font-semibold"><span className="block">Inventory item / item name *</span>{item.custom_item ? <><input autoFocus required className={inputClass} value={item.name} onChange={(event) => updateItem(index, { name: event.target.value })} placeholder="Enter a new item name" /><button type="button" className="mt-1 text-orange-700 underline" onClick={() => updateItem(index, { custom_item: false, name: '' })}>Choose an inventory item</button></> : <SearchableSelect value={item.inventory_item_id || ''} onChange={(id) => chooseInventoryItem(index, id)} options={itemOptions} placeholder="Search items or create one..." />}</label><label className="block space-y-1 font-semibold"><span className="block">Quantity *</span><input required type="number" min="0.001" step="0.001" className={inputClass} value={item.quantity} onChange={(event) => updateItem(index, { quantity: event.target.value })} /></label><label className="block space-y-1 font-semibold"><span className="block">Unit cost *</span><input required type="number" min="0" step="0.01" className={inputClass} value={item.unit_cost} onChange={(event) => updateItem(index, { unit_cost: event.target.value })} /></label><button type="button" onClick={() => setItems((rows) => rows.filter((_, i) => i !== index))} className="justify-self-start font-semibold text-red-700">Remove item</button></div>)}
        </section>
        ...
        <div className="flex justify-end gap-2 border-t pt-3"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2 font-semibold">Cancel</button><button type="submit" disabled={busy} className="rounded-lg bg-orange-600 px-5 py-2 font-bold text-white hover:bg-orange-700 disabled:opacity-50">{busy ? 'Submitting…' : 'Submit expense to Finance'}</button></div>
      </form>
    </section>
  </div>;
}
