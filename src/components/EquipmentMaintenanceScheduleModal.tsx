...
export default function EquipmentMaintenanceScheduleModal({ assets, employees, projectId, onClose, onSaved }: {
  ...
  return (
    ...
    >
      <form id="equipment-maintenance-form" onSubmit={submit} className="space-y-4 text-xs">
        ...
        <section className="space-y-3 rounded-xl border p-3">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="space-y-1 font-semibold">Maintenance type *<SearchableSelect value={form.maintenance_type} onChange={(value) => setForm({ ...form, maintenance_type: value })} options={[['PREVENTIVE','Preventive maintenance (PM)'],['CORRECTIVE','Corrective repair'],['INSPECTION','Safety inspection & audit'],['OVERHAUL','Major component overhaul']].map(([value,label]) => ({ value, label }))} placeholder="Select maintenance type..." /></label>
            <label className="space-y-1 font-semibold">Component system / failure taxonomy *<SearchableSelect value={form.failure_taxonomy} onChange={(value) => setForm({ ...form, failure_taxonomy: value })} options={[['GENERAL','General PM / lubrication'],['HYDRAULIC','Hydraulic system'],['ENGINE','Engine & drivetrain'],['ELECTRICAL','Electrical & instrumentation'],['PNEUMATIC','Pneumatic & air compressor'],['STRUCTURAL','Structural & mast chassis']].map(([value,label]) => ({ value, label }))} placeholder="Select failure taxonomy..." /></label>
            <label className="space-y-1 font-semibold">Priority level *<SearchableSelect value={form.priority} onChange={(value) => setForm({ ...form, priority: value })} options={[['LOW','Low priority'],['NORMAL','Normal priority'],['HIGH','High priority'],['CRITICAL','Critical / downtime risk']].map(([value,label]) => ({ value, label }))} placeholder="Select priority..." /></label>
            <label className="space-y-1 font-semibold">Recurrence frequency<SearchableSelect value={form.recurrence} onChange={(value) => setForm({ ...form, recurrence: value })} options={[['ONE_OFF','One-time service'],['EVERY_250_HOURS','Every 250 engine hours'],['EVERY_500_HOURS','Every 500 engine hours'],['WEEKLY','Weekly schedule'],['MONTHLY','Monthly schedule'],['QUARTERLY','Quarterly audit']].map(([value,label]) => ({ value, label }))} placeholder="Select recurrence..." /></label>
          </div>
        </section>
        <section className="space-y-3 rounded-xl border bg-slate-50 p-3 dark:bg-slate-800/40">
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="space-y-1 font-semibold">Scheduled start date *<AppDateTimePicker mode="date" required value={form.scheduled_date} onChange={(val) => setForm({ ...form, scheduled_date: val })} /></label>
