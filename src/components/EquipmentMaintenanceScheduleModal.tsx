...
export default function EquipmentMaintenanceScheduleModal({ assets, employees, projectId, onClose, onSaved }: {
  ...
  return <Modal title="Create & Dispatch Equipment Maintenance Schedule" onClose={onClose} className="max-w-3xl">
    <form onSubmit={submit} className="space-y-4 px-1 text-xs max-h-[78vh] overflow-y-auto">
      ...
      <div className="flex justify-end gap-2 border-t pt-3"><button type="button" onClick={onClose} className="rounded-lg border px-4 py-2 font-medium">Cancel</button><button type="submit" disabled={busy} className="rounded-lg bg-orange-600 px-4 py-2 font-bold text-white hover:bg-orange-700 disabled:opacity-50">{busy ? 'Creating schedule…' : 'Create & dispatch maintenance schedule'}</button></div>
    </form>
  </Modal>;
}
