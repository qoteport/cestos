...
export function AuthProvider({children}:{children:ReactNode}) {
 ...
 }catch(e){if(e instanceof ApiError && (e.status===401 || /bearer\s*token\s*required/i.test(e.message))){setUser(null);setAccess(null);localStorage.removeItem('cestos_portal_type');localStorage.removeItem('cestos_is_superuser');if(typeof window!=='undefined'&&!window.location.pathname.startsWith('/sign-up-login')){window.location.href='/sign-up-login';}}else setError(e instanceof Error?e.message:'Could not restore session.');}finally{setLoading(false);}},[]);
 useEffect(()=>{if(window.location.hash.startsWith('#reset=')&&window.location.pathname!=='/sign-up-login'){window.location.replace('/sign-up-login'+window.location.hash);return;}void reload();const expired=()=>{setUser(null);setAccess(null);setLoading(false);localStorage.removeItem('cestos_portal_type');localStorage.removeItem('cestos_is_superuser');if(typeof window!=='undefined'&&!window.location.pathname.startsWith('/sign-up-login')){window.location.href='/sign-up-login';}};const changed=(e:StorageEvent)=>{if(e.key?.startsWith('cestos_'))void reload();};window.addEventListener('cestos:session-expired',expired);window.addEventListener('storage',changed);return()=>{window.removeEventListener('cestos:session-expired',expired);window.removeEventListener('storage',changed);};},[reload]);
