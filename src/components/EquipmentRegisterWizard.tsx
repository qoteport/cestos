...
export default function EquipmentRegisterWizard({ projectId, assets, record, onClose, onSaved }: {
  ...
  const input = (key: string, label: string) => <label key={key} className="block space-y-1.5"><span className="block text-xs font-semibold text-slate-700">{label}{key === 'equipment' ? ' *' : ''}</span>{['open_defects', 'action_required', 'remarks'].includes(key) ? <textarea className="input-field min-h-24 w-full rounded-none border-slate-300 focus-visible:ring-2 focus-visible:ring-[#184877]" value={data[key] || ''} onChange={(e) => set(key, e.target.value)} /> : <input className="input-field w-full rounded-none border-slate-300 focus-visible:ring-2 focus-visible:ring-[#184877]" value={data[key] || ''} onChange={(e) => set(key, e.target.value)} />}</label>;
  ...
  const choice = (key: 'status' | 'priority', label: string, choices: string[], isCustom: boolean, setCustom: (value: boolean) => void) => {
    ...
    return (
      <label className="block space-y-1">
        ...
        {mode === 'FREE_FLOW' ? (
          <input className="input-field w-full" value={data[key] || ''} onChange={(e) => set(key, e.target.value)} />
        ) : isCustom ? (
