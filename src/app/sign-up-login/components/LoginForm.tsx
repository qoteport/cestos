  ...
  async function submit(e: FormEvent) {
    ...
    try {
      if (token) {
        ...
        await apiFetch('/api/v1/hr/password-reset', { method: 'POST', body: JSON.stringify({ token, password }) }, false);
        setToken('');
        setPassword('');
        setConfirm('');
    ...
    <div className="flex-1 flex flex-col items-center justify-center px-6 py-12 bg-background overflow-y-auto">
      <div className="w-full max-w-md">
        ...
        <h2 className="text-2xl font-bold mt-3">
          {token ? 'Set your password' : 'Welcome back'}
        </h2>
        ...
        {message && (
          <p role="status" className="mb-5 rounded bg-secondary p-3 text-sm text-primary">
            {message}
          </p>
        )}
        ...
        <p className="text-xs text-muted-foreground mt-6">
          Access and available actions follow your organization’s permissions.
        </p>
      </div>
    </div>
  );
